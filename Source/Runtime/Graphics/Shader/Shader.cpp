...
Shader::Shader(std::shared_ptr<GraphicsDevice> device, const ShaderDesc& desc)
{
	...
	std::string shaderBlod = ShaderReader::ReadShaderFile(mShaderName);

	D3DCompile(shaderBlod.data(), shaderBlod.length(), nullptr, nullptr, nullptr, mEntryPoint.c_str(), 
		mShaderModel.c_str(), D3DCOMPILE_ENABLE_STRICTNESS, 0, mShaderBlob.GetAddressOf(), &errorBlob);

	if (errorBlob.Get() != nullptr && errorBlob->GetBufferSize() > 0)
	{
